...
unsigned __stdcall threadReceive1(void* sc)
{
	...
	while (1)
	{
		...
		{
			if (strcmp(buffer1, "exit()") == 0||strcmp(buffer2,"exit()")==0)
			{
				cout << "线程1即将退出..." << endl;
				send(cs->clientSocket2, "exit()", sizeof(buffer1), 0);
				/*closesocket(cs->serverSocket);
				closesocket(cs->clientSocket1);*/
				ExitThread(0);
				return 0;
...
unsigned __stdcall threadReceive2(void* sc)
{
	...
	while (1)
	{
		...
		{
			if (strcmp(buffer2, "exit()") == 0||strcmp(buffer1,"exit()")==0)
			{
				cout << "线程2即将退出..." << endl;
				send(cs->clientSocket1, "exit()", sizeof("exit()"), 0);
				/*closesocket(cs->serverSocket);
				closesocket(cs->clientSocket1);
				closesocket(cs->clientSocket2);*/
				ExitThread(0);
